    ...
    return (
        <section className="home-projects py-[40px] md:py-[80px]">
            <div className="primary-container">
                <SectionTitle title="From the Projects" link="/projects" />

                <motion.div
                    ref={ref}
                    className="flex items-center justify-center gap-6 max-lg:flex-wrap md:gap-[32px] lg:flex-row"
                ...
                >
                    {projectsData.map((project, index) => (
                        <motion.div key={index} variants={cardVariants}>
                            <MyCard imgUrl={project.imgUrl} title={project.title} excerpt={project.excerpt} />
                        </motion.div>
                    ))}
                </motion.div>
            </div>
