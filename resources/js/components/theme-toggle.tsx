...
export default function ThemeToggle({ variant = 'icon-only', className = '' }: ThemeToggleProps) {
    ...
    return (
        ...
        >
            {isDark ? (
                <Sun className="h-5 w-5 transition-transform duration-200 hover:scale-110 hover:rotate-12" />
            ) : (
                <Moon className="h-5 w-5 transition-transform duration-200 hover:scale-110 hover:-rotate-12" />
            )}
        </button>
    );
}
