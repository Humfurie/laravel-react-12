...
export default function Edit({ raffle }: Props) {
    ...
    const handleSubmit: FormEventHandler = (e) => {
        e.preventDefault();
        put(route('admin.raffles.update', raffle.slug));
    };
    ...
    const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setUploading(true);
        router.post(route('admin.raffles.images.upload', raffle.slug), formData, {
            onFinish: () => {
                setUploading(false);
    ...
    const handleDeleteImage = (imageId: number) => {
        if (confirm('Are you sure you want to delete this image?')) {
            router.delete(route('admin.raffles.images.delete', [raffle.slug, imageId]));
        }
    };

    const handleSetPrimaryImage = (imageId: number) => {
        router.patch(route('admin.raffles.images.set-primary', [raffle.slug, imageId]));
    };
    ...
    const handleClaimPrize = () => {
        ...
        router.post(
            route('admin.raffles.claim-prize', raffle.slug),
            {},
            {
    ...
    const handleRejectWinner = () => {
        ...
        setSubmittingClaim(true);
        router.post(
            route('admin.raffles.reject-winner', raffle.slug),
    ...
    const filteredEntries = useMemo(() => {
        if (!searchQuery.trim()) return raffle.entries;
        return raffle.entries.filter((entry) => entry.name.toLowerCase().includes(searchQuery.toLowerCase()));
    }, [searchQuery, raffle.entries]);

    return (
        <AdminLayout>
            <Head title={`Edit Raffle - ${raffle.title}`} />
            ...
            <div className="container mx-auto max-w-5xl py-8">
                <div className="mb-8 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                        {raffle.entries_count > 0 && !raffle.winner && (
                        ...
                        {raffle.entries_count > 0 && (
                            <div className="bg-card rounded-lg border p-6">
                                <div className="mb-4 flex items-center justify-between">
                                    <h2 className="text-xl font-semibold">Entries ({raffle.entries_count})</h2>
                                </div>
                    ...
                    <div className="space-y-6">
                        <div className="bg-card rounded-lg border p-6">
                            ...
                            <div className="space-y-3">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span className="font-semibold">{raffle.entries_count}</span>
                                </div>
                                <div className="flex items-center justify-between">
                            ...
                            <div className="bg-card rounded-lg border p-6">
                                ...
                                <div className="space-y-2">
                                    <div>
                                        ...
                                        <p className="font-medium">{raffle.winner.name}</p>
                                    </div>
                                    <div>
                                        <p className="text-muted-foreground text-sm">Phone</p>
                                        <p className="font-medium">{raffle.winner.phone}</p>
                                    </div>
                                    <div>
                                        <p className="text-muted-foreground text-sm">Facebook</p>
                                        <a
                                            href={raffle.winner.facebook_url}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                    ...
                                    <div className="space-y-3">
                                        ...
                                        {raffle.rejection_reason && (
                                            <div className="rounded-md border border-red-200 bg-red-50 p-3">
                                                ...
                                                <p className="mt-1 text-xs text-red-600">{raffle.rejection_reason}</p>
                                            </div>
                                        )}
                        ...
                        <div className="bg-card rounded-lg border p-6">
                            ...
                            <div className="space-y-2">
                                ...
                                <Button variant="outline" className="w-full" onClick={() => window.open(`/raffles/${raffle.slug}`, '_blank')}>
