...
export default function WinnerSelection({ raffle }: Props) {
    ...
    const handleSelectWinner = () => {
        ...
        const spinInterval = setInterval(() => {
            setCurrentIndex((prev) => (prev + 1) % raffle.entries.length);
            iterations++;
            ...
            if (iterations >= maxIterations) {
                ...
                setTimeout(() => {
                    ...
                    router.post(
                        route('admin.raffles.select-winner', raffle.slug),
                        {},
                        {
                            preserveState: false,
                        },
    ...
    }, [isSpinning, spinSpeed, raffle.entries.length]);

    const currentEntry = raffle.entries[currentIndex];

    return (
        <AdminLayout>
            <Head title={`Winner Selection - ${raffle.title}`} />

            <div className="container mx-auto max-w-4xl py-8">
                <div className="mb-8">
                    <Button variant="ghost" onClick={() => router.visit(route('admin.raffles.edit', raffle.slug))} className="mb-4">
                ...
                {raffle.winner ? (
                    <div className="bg-card rounded-lg border p-8 text-center">
                        ...
                        <div className="bg-muted/50 mx-auto mt-6 max-w-md space-y-4 rounded-lg border p-6">
                            <div>
                                ...
                                <p className="text-xl font-semibold">{raffle.winner.name}</p>
                            </div>
                            <div>
                    ...
                    <div className="flex min-h-[400px] flex-col items-center justify-center rounded-lg border border-dashed p-8 text-center">
                        ...
                        <p className="text-muted-foreground text-sm">
                            There are no entries for this raffle. Wait for participants to enter before selecting a winner.
                        </p>
                    </div>
                ) : (
                    <div className="space-y-6">
                        <div className="bg-card rounded-lg border p-8">
                            <div className="mb-6 text-center">
                                <Badge variant="secondary" className="mb-2">
                                    ...
                                    {raffle.entries.length} Total Entries
                                </Badge>
                            </div>
                        ...
                        <div className="bg-card rounded-lg border p-6">
                            ...
                            <div className="max-h-[400px] space-y-2 overflow-y-auto">
                                {raffle.entries.map((entry, index) => (
                                    <div
                                        key={entry.id}
