...
export default function Entries({ raffle, entries }: Props) {
    ...
    return (
        <>
            <Head title={`Entries - ${raffle.title}`} />
                    ...
                    <div className="mb-8 text-center">
                        {raffle.primary_image_url && (
                            <div className="mb-6 flex justify-center">
                                <img src={raffle.primary_image_url} alt={raffle.title} className="h-32 w-32 rounded-full object-cover shadow-lg" />
                            </div>
                        )}
                        <h1 className="mb-2 text-3xl font-bold">{raffle.title}</h1>
                        <div className="text-muted-foreground flex items-center justify-center gap-2">
                            <Users className="h-5 w-5" />
                            <span>{raffle.entries_count} Total Entries</span>
                        </div>
                    </div>
                            ...
                            {filteredEntries.length === 0 ? (
                                <div className="flex min-h-[400px] flex-col items-center justify-center p-8 text-center">
                                    ...
                                    <p className="text-muted-foreground">
                                        {searchQuery ? 'Try a different search term' : 'Be the first to enter this raffle!'}
                                    </p>
                                </div>
                            ) : (
                                <div
